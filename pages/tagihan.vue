...
const selectedServiceData = computed(() => {
  return bills.value.find((service) => service.id === selectedService.value);
});
...
<template>
  <Card>
    <CardHeader>
      ...
      <CardDescription>Bayar tagihan bulanan Anda dengan mudah</CardDescription>
    </CardHeader>
    <CardContent class="space-y-6">
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <Card
          v-for="service in bills"
          :key="service.id"
          ...
          <CardContent class="p-0">
            <div class="relative overflow-hidden">
              ...
              <div class="absolute top-2 right-2">
                <span class="inline-block px-2 py-1 bg-black/70 text-white text-xs rounded-full backdrop-blur-sm"> Tagihan </span>
              </div>
            </div>
      ...
      <div v-if="selectedServiceData" class="mt-8 space-y-6">
        <div class="flex items-center gap-4">
          ...
          <Badge class="bg-yellow-500 text-white"> Tagihan </Badge>
        </div>

        <!-- Varian Layanan -->
        <div class="space-y-4">
          <h4 class="text-lg font-semibold text-gray-900">Pilih Layanan</h4>
        ...
        <div v-if="selectedVariant" class="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p class="text-green-800 font-medium">
            ✓ Layanan terpilih: {{ selectedServiceData.variants.find((v) => v.code === selectedVariant)?.name }}
          </p>
          <p class="text-green-600 text-sm mt-1">
            Biaya Admin: Rp {{ selectedServiceData.variants.find((v) => v.code === selectedVariant)?.price.value.toLocaleString('id-ID') }}
          </p>
        </div>
        ...
        <div v-if="selectedVariant" class="mt-8 space-y-6">
          ...
          <Card>
            <CardHeader>
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-yellow-500 text-white rounded-full flex items-center justify-center font-bold text-sm">4</div>
                <div>
                  <CardTitle class="text-lg">Masukkan detail tagihan</CardTitle>
                  <div class="flex items-center gap-2 mt-1">
                    <span class="w-2 h-2 bg-yellow-500 rounded-full"></span>
                    <span class="text-sm text-yellow-600 font-medium">Panduan</span>
                  </div>
                </div>
              </div>
            </CardHeader>
            <CardContent class="space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div v-for="input in selectedServiceData.inputs" :key="input.name" class="space-y-2">
                  ...
                  <Select v-if="input.type === 'select'" v-model="formData[input.name]">
                    <SelectTrigger class="w-full">
                      <SelectValue :placeholder="`Pilih ${input.title}`" />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Input
                    ...
                    :type="input.type"
                    :placeholder="`Masukkan ${input.title}`"
                    class="w-full"
                  />
                </div>
              </div>
              ...
              <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p class="text-sm text-yellow-800">
                  ...
                  <button class="text-yellow-600 underline ml-1">Selengkapnya</button>
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div class="flex justify-end">
            <button
              class="px-6 py-3 bg-yellow-600 text-white rounded-lg font-semibold hover:bg-yellow-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
